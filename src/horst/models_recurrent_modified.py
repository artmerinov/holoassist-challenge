...
class AttentionRNNCell(nn.Module):
    def __init__(self,
        ...
        kernel_size = 3, bias = True, stride = 1, output_ht=False, frame_dropout=0):
    ...
    def forward(self, inputs, first_step = False, checkpointing = False, mask = None):
        ...
        return outputs, ht, self.output_queue[-1][mask] + q[0]
...
class ConvLSTM(nn.Module):
    def __init__(self,
        ...
        Cell = lambda in_channels, out_channels, stride, output_ht, frame_dropout: AttentionRNNCell(
                input_channels = in_channels, hidden_channels = out_channels,
                order = cell_params["order"], steps = cell_params["steps"], ranks = cell_params["ranks"], output_ht = output_ht,
        ...
        for b in range(self.num_blocks):
            for l in range(layers_per_block[b]):
                ...
                self.layers[lid] = Cell(channels, hidden_channels[b], self.stride[b], output_ht[b], frame_dropout)
