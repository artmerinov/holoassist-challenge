...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.layer1 = self._make_layer(block, 64, blocks=layers[0], num_segments=num_segments, gsf_ch_ratio=gsf_ch_ratio)
        self.layer2 = self._make_layer(block, 128, blocks=layers[1], stride=2, dilate=replace_stride_with_dilation[0], num_segments=num_segments, gsf_ch_ratio=gsf_ch_ratio)
        self.layer3 = self._make_layer(block, 256, blocks=layers[2], stride=2, dilate=replace_stride_with_dilation[1], num_segments=num_segments, gsf_ch_ratio=gsf_ch_ratio)
        self.layer4 = self._make_layer(block, 512, blocks=layers[3], stride=2, dilate=replace_stride_with_dilation[2], num_segments=num_segments, gsf_ch_ratio=gsf_ch_ratio)
...
def wide_resnet50_2(pretrained=False, progress=True, num_segments=8, gsf_ch_ratio=25, **kwargs):
