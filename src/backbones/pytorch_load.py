...
from .layer_factory import get_basic_layer, parse_expr, build_gsf, build_gsm, build_tsm
        ...
        for l in layers:
            ...
            if op != 'Concat':
                id, out_name, module, out_channel, in_name = get_basic_layer(
                    l, 3 if len(self._channel_dict) == 0 else self._channel_dict[in_var[0]], conv_bias=True)
    ...
    def __init__(self, 
        ...
        for l in layers:
            ...
            if op != 'Concat':
                if op == 'gsm':
                    gsm_cnt += 1
                    id, out_name, module, out_channel, in_name = build_gsm(
                        l,  self._channel_dict[in_var[0]], conv_bias=True, num_segments=num_segments)
                    self._channel_dict[out_name] = out_channel
                    setattr(self, id, module)
                    self._op_list.append((id, op, out_name, in_name))
                else:
    ...
    def __init__(self, model_path='src/backbones/bn_inception_gsf.yaml',
        ...
        for l in layers:
            ...
            if op != 'Concat':
                if op == 'gsf':
                    gsf_cnt += 1
                    id, out_name, module, out_channel, in_name = build_gsf(
                        l,  num_segments=num_segments, gsf_ch_ratio=gsf_ch_ratio)
                    self._channel_dict[out_name] = out_channel
                    setattr(self, id, module)
                    self._op_list.append((id, op, out_name, in_name))
                else:
...
class BNInception_tsm(nn.Module):
    def __init__(self, 
                 model_path='src/backbones/bn_inception_tsm.yaml',
                 weight_url='https://yjxiong.blob.core.windows.net/models/bn_inception-9f5701afb96c8044.pth',
                 num_segments=8,
                 n_div=8):
        super(BNInception_tsm, self).__init__()
        ...
        for l in layers:
            ...
            if op != 'Concat':
                if op == 'tsm':
                    tsm_cnt += 1
                    id, out_name, module, out_channel, in_name = build_tsm(
                        l,  num_segments=num_segments, n_div=n_div)
                    self._channel_dict[out_name] = out_channel
                    setattr(self, id, module)
                    self._op_list.append((id, op, out_name, in_name))
                else:
        ...
        for k, v in state_dict.items():
            state_dict[k] = torch.squeeze(v, dim=0)
        self.load_state_dict(state_dict, strict=False)
        print('No. of TSM modules = {}'.format(tsm_cnt))
...
class InceptionV3_tsm(BNInception_tsm):
    def __init__(self, 
                 model_path='src/backbones/inceptionv3_tsm.yaml',
                 weight_url='https://yjxiong.blob.core.windows.net/models/inceptionv3-cuhk-0e09b300b493bc74c.pth',
                 num_segments=8,
                 n_div=8):
        super(InceptionV3_tsm, self).__init__(model_path=model_path, weight_url=weight_url, num_segments=num_segments,
                                              n_div=n_div)
